...
//[TODO:huizhi]we need force copy to overwrite without confirmation
func (ss *SSH) CopyForMD5(host, localFilePath, remoteFilePath, md5 string) bool {
	...
	if remoteMD5 == md5 {
		logger.Debug("[ssh]md5 validate true: %s", remoteMD5)
		return true
	}
	logger.Error("[ssh]md5 validate false. remoteMD5 is %s, localmd5 is %s", remoteMD5, md5)
	return false
}
...
func (ss *SSH) Copy(host, localFilePath, remoteFilePath string) {
	...
	for {
		...
		logger.Debug("[ssh][%s]transfer total size is: %.2f%s ;speed is %d%s", host, totalLength, totalUnit, speed, unit)
	}
}
...
func (ss *SSH) CopyConfigFile(host, remoteFilePath string, localFilePathOrBytes interface{}) {
	...
	for {
		...
		logger.Debug("[ssh][%s]transfer total size is: %d%s", host, totalMB, "MB")
	}
}
