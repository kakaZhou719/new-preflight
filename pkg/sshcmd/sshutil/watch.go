...
func (ss *SSH) LoggerFileSize(host, filename string, size int) {
	...
	for {
		...
		case <-t.C:
			...
			logger.Debug("[ssh][%s]transfer total size is: %.2f%s", host, value, "MB")
		}
	}
}
...
//RemoteFileExist is
func (ss *SSH) IsFileExistAndEqual(host, remoteFilePath, desiredMd5 string) bool {
	return ss.IsFileExist(host, remoteFilePath) && (ss.Md5sum(host, remoteFilePath) == desiredMd5)
}

func (ss *SSH) Md5sum(host, remoteFilePath string) string {
	cmd := fmt.Sprintf("md5sum %s | cut -d\" \" -f1", remoteFilePath)
	md5 := ss.CmdToString(host, cmd, "")
